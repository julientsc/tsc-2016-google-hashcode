...
public class SimulationF {
	...
	public SimulationF(DataLoader dataLoader, String output) {
				...
				if(drone.getCostOptimized()==minCurrentCost) {
					...
					if(orders.size() == 0) {
						for(Warehouse warehouse : dataLoader.getWarehouses()) {
							orders = getOrderInOneStep(warehouse);
							if(orders.size() > 0) {
								dronesRepartition.put(drone, warehouse);
								currentDroneWarehouse = warehouse;
								break;
							}
						}
					}
