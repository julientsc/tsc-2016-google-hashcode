...
public class SimulationMotherOptimizer {
	...
	public SimulationMotherOptimizer(DataLoader dataLoader, String output) {
		...
		do {
			for (Drone drone : dronesRepartition.keySet()) {
				if(drone.getCostOptimized()==minCurrentCost) {
					...
					orders = new ArrayList<>(); // getOrderInOneStep(currentDroneWarehouse);
